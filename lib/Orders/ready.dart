...
class Ready extends StatefulWidget {
  ...
  const Ready({Key key, this.vendorDetails, this.orderTotal}) : super(key: key);
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: FutureBuilder<List<OrderTotalModel>>(
          future: allApi.getOrderStatus(status: "Ready", vid: widget.vendorDetails.vendorId),
          builder: (context, snapshot) {
            if (!snapshot.hasData) {
              ...
              return const Text('There are no orders ready at the moment.');
            } else {
      ...
      onTap: () {
        Get.to(
          OrderTotal(
            ...
            savings: savings,
            total: total,
            wallet: wallet,      vendorDetails:widget.vendorDetails,status: 'Ready',address: address,
          ),
        );
      },
      child: Container(
